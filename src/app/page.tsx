...
export default function LandingPage() {
  ...
  const startDemoMode = () => {
    toast({ title: "Demo Mode", description: "Launching test environment with generated data..." })
    // In a real app, you might use a specific test account or local state
    setAuthMode('login')
    setEmail('demo@medistay.ai')
    ...
    <div className="flex flex-col min-h-screen bg-background text-foreground font-sans">
      ...
      <main className="flex-1">
            ...
            <div className="space-y-8">
              ...
              <div className="flex gap-12">
                <div>
                  ...
                  <p className="text-slate-400 font-bold text-sm">Commissions Saved</p>
                </div>
                <div>
                  <p className="text-4xl font-black text-primary mb-1">94%</p>
                  <p className="text-slate-400 font-bold text-sm">Avg. Retention Rate</p>
                </div>
              </div>
            </div>
              ...
              {reviews.map((rev, i) => (
                <Card key={i} className="bg-white/5 border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-sm">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-black text-lg text-white">{rev.name}</p>
                      <p className="text-primary text-sm font-bold uppercase tracking-widest">{rev.role}</p>
                    </div>
                  </div>
                </Card>
              ))}
        ...
        <section className="py-24 px-8 max-w-6xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              { title: 'MARx Switch Detection', desc: 'Instantly detect carrier switches via nightly snapshot polls.', icon: Activity },
              { title: 'EDI Operations', desc: 'Secure X12 handshakes via Stedi Core for real-time eligibility.', icon: Terminal },
              { title: 'Autonomous LIS Scanner', desc: 'Automatically identify members eligible for federal subsidies.', icon: Zap },
              { title: 'AEP Shield', desc: 'Orchestrate loyalty campaigns before the disenrollment window opens.', icon: ShieldCheck },
            ].map((feature, i) => (
              <Card key={i} className="p-10 rounded-[2.5rem] border border-border space-y-6 hover:border-primary/30 transition-all group cursor-default">
      ...
      <footer className="py-20 px-8 border-t border-border/50 text-center">
        ...
        <div className="flex justify-center gap-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">
          ...
          <Link href="#" className="hover:text-primary transition-colors">BAA Agreement</Link>
        </div>
      </footer>
      ...
      {authMode && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-8">
          ...
          <Card className="relative w-full max-w-md rounded-[3rem] shadow-2xl p-12 text-center animate-in zoom-in duration-300">
            ...
            <h2 className="text-3xl font-black mb-2 text-slate-900">{authMode === 'login' ? 'Agency Access' : 'Provision Node'}</h2>
            <p className="text-center text-slate-400 text-sm font-medium mb-8 uppercase tracking-widest">MediStay Retention OS</p>
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              ...
              <Button type="submit" disabled={loading} className="w-full h-16 rounded-3xl bg-slate-900 hover:bg-slate-800 text-white font-black text-lg mt-4 shadow-xl">
