...
import { Search, Filter, ArrowUpRight, ShieldCheck, AlertCircle, MoreHorizontal } from "lucide-react"
      ...
      <div className="flex-1 flex flex-col h-full bg-background">
        <header className="h-16 border-b border-border px-8 flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-10">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" className="rounded-xl h-9 text-xs font-bold">Export CSV</Button>
            <Button className="rounded-xl h-9 text-xs font-bold bg-secondary hover:bg-secondary/90 shadow-sm" asChild>
              <Link href="/members/new">Enroll New</Link>
            </Button>
          </div>
        </header>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="rounded-xl h-10 gap-2 border-dashed">
                <Filter className="w-4 h-4" /> Carrier
              </Button>
              <Button variant="outline" size="sm" className="rounded-xl h-10 gap-2 border-dashed">
                <Filter className="w-4 h-4" /> Risk Level
              </Button>
            </div>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/50 hover:bg-muted/50">
                  <TableHead className="font-bold text-[10px] uppercase tracking-widest h-12 px-6">Member</TableHead>
                  <TableHead className="font-bold text-[10px] uppercase tracking-widest">Plan & Carrier</TableHead>
                  <TableHead className="font-bold text-[10px] uppercase tracking-widest">Enrollment</TableHead>
                  <TableHead className="font-bold text-[10px] uppercase tracking-widest">Status</TableHead>
                  <TableHead className="font-bold text-[10px] uppercase tracking-widest">Score</TableHead>
                  <TableHead className="text-right font-bold text-[10px] uppercase tracking-widest px-6">Action</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredMembers.map((member) => (
                  <TableRow key={member.id} className="hover:bg-primary/5 transition-colors border-border/50">
                    ...
                    <TableCell>
                      <Badge variant="outline" className="text-[9px] uppercase font-bold">{member.enrollmentPeriod}</Badge>
                    </TableCell>
                    <TableCell>
                      <Badge 
                    ...
                    <TableCell className="text-right px-6">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <Button variant="outline" size="sm" asChild className="h-8 text-[10px] font-black uppercase tracking-tighter border-primary/20 text-primary hover:bg-primary/5 rounded-lg">
