...
import { Download, ShieldAlert, History, User } from "lucide-react"
      ...
      <div className="flex-1 flex flex-col h-full bg-background">
        <header className="h-16 border-b border-border px-8 flex items-center justify-between bg-white/50 backdrop-blur-md sticky top-0 z-10">
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-headline font-semibold text-primary">{member.fullName}</h1>
            <Badge variant={member.status === 'churn-risk' ? 'destructive' : 'secondary'} className="rounded-full uppercase text-[10px] px-3">
              {member.status}
            </Badge>
          </div>
            ...
            <TabsContent value="overview" className="space-y-6 mt-6">
                ...
                <div className="p-6 rounded-2xl border border-border bg-white space-y-4">
                  ...
                  <dl className="space-y-3">
                    <div className="flex justify-between text-sm border-b pb-2">
                      ...
                      <dd className="font-semibold">{member.age} years</dd>
                    </div>
                    <div className="flex justify-between text-sm border-b pb-2">
                      <dt className="text-muted-foreground">Medicare Status</dt>
                      <dd className="font-semibold">{member.medicareMedicaidStatus}</dd>
                    </div>
                    <div className="flex justify-between text-sm border-b pb-2">
                      <dt className="text-muted-foreground">Last Review</dt>
                      <dd className="font-semibold">{member.lastReviewDate}</dd>
                    </div>
                    <div className="flex justify-between text-sm border-b pb-2">
                      <dt className="text-muted-foreground">Ownership ID</dt>
                      <dd className="font-mono text-xs opacity-50">{member.agentId}</dd>
                    </div>
                  </dl>
                </div>
              ...
              <div className="p-8 rounded-2xl border border-border bg-white space-y-4">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Member is approaching the 12-month inactivity threshold. System has flagged a potential Medicare transition review for the upcoming quarter. Automated PDF summary for last review period is available in the Vault.
                </p>
              </div>
            </TabsContent>
            ...
            <TabsContent value="timeline">
              <div className="space-y-4">
                {[
                  ...
                  { date: "2024-10-01", action: "Member Record Created", agent: "Current Agent" }
                ].map((item, i) => (
