...
export function InsightsPanel({ member }: InsightsPanelProps) {
  ...
  const handleTriggerFax = () => {
    if (!member) return;
    toast({ title: "Module 3: SSBCI Fax Agent", description: "Generating HIPAA package & faxing physician via Spruce Health..." })
  ...
  if (!member) return (
    <aside className="w-80 border-l border-border bg-card p-6 hidden lg:flex flex-col gap-6 shadow-2xl relative z-20">
      <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
        ...
        <div>
          <h3 className="text-sm font-black uppercase text-foreground">MediStay Intelligence</h3>
        ...
        <div className="space-y-6">
            ...
            <Card className="rounded-2xl border-destructive/20 bg-destructive/5 shadow-sm overflow-hidden">
              ...
              <CardContent className="space-y-3">
                <p className="text-[10px] text-destructive leading-relaxed font-bold uppercase">
                  Disenrollment pending on MARx snapshot. New Carrier identified. 24h window active.
                </p>
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-black text-[10px] text-foreground uppercase tracking-tight">SSBCI Fax Agent</span>
                  </div>
                  <Badge variant="outline" className="text-[8px] uppercase font-black px-1.5">{member.ssbciStatus}</Badge>
                </div>
          ...
          <div className="pt-4 mt-auto space-y-3">
             ...
             <p className="text-[8px] text-center text-muted-foreground font-bold uppercase tracking-tighter opacity-50 px-2">
               MANAGED AUTONOMOUSLY BY MEDISTAY AI. PHI REMAINS AGENT-SCOPED AND ENCRYPTED AT REST.
             </p>
          </div>
        </div>
      )}
